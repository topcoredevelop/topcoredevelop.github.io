...
function Work({ position, company, location, type, duration, descriptions }) {
    ...
    <>
      <article className="pt-8 border-b-2 border-dark-content pb-5 dark:border-light-content border-opacity-20 dark:border-opacity-20">
        <div className="flex justify-between items-center">
          <h1 className=" md:text-lg lg:text-xl">{position}</h1>
        ...
        <ul class="list-group  list-group-flush">
          {
            descriptions !== undefined && React.Children.toArray(
              descriptions.map((description) => (
                <li className="text-content p-5 py-3 lg:max-w-5xl">{description}</li>
              ))
            )
          }
        </ul>
